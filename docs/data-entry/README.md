# Pertinence Dashboard — Data Entry Standards

This folder is the handover guide for the Google Sheets that feed the Pertinence Dashboard. Each team has its own short document. **Everyone** reads the Common Rules first.

## Read first

- **[00-common.md](00-common.md) — Common Rules.** How the dashboard reads sheets, universal rules that apply to every sheet, the process for adding new locations / purposes / categories, troubleshooting, and the glossary. ~10 minutes; read once.

## By department

| Department | Document | Sheet(s) you own |
| --- | --- | --- |
| Sales & Finance | [01-sales-and-finance.md](01-sales-and-finance.md) | Bank Deposit Mirror (tabs: `2026 LAND`, `2026 Weekly Sales Report`, `2026 Customer File`) |
| Marketing | [02-marketing.md](02-marketing.md) | Marketing Fund Expense Sheet (one tab per month) |
| Customer Support | [03-customer-support.md](03-customer-support.md) | MASTER SHEET - CUSTOMER SUPPORT (one tab per rep) |
| Realtor Management | [04-realtor-management.md](04-realtor-management.md) | Marketing Team Reporting Template (tab: `2026 Realtors Managers Weekly Report`) |

## How to use these documents

1. Read **00-common.md** once. Keep it findable — it's the rule book.
2. Read your team's document in full. Refer back when you're entering new types of data or when something looks wrong on the dashboard.
3. When in doubt, **do not "fix" a value to look right on the dashboard**. The dashboard's Data Quality view exists to surface unusual values for the supervisor to decide on. Editing a sheet to match the dashboard hides real problems.
4. If a sheet column, header, or tab needs to change, contact the dashboard owner **before** changing it — even a renamed header can hide a whole column from the dashboard.

---

*Owner: Pertinence Group Operations. Last reviewed: 2026-05-29.*
