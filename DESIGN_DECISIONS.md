...
## Realtor Management panel scope (v1, locked 2026-05-14; ingest landed 2026-05-25)
...
- **Schema keeps `realtor_managers` (Mrs Kemi / Richard Makava / Debbie) reference table** for any future per-manager UI. The original `realtor_manager_weekly` fact table (migration 004, wrong-shape — typed cols + NOT NULL `realtor_manager_id`) was **dropped in migration 018** and replaced with `realtor_metrics_monthly` at grain `(period_year, period_month, metric_key)` plus `realtor_metric_canonicals` (19 canonical metrics across 3 categories) and `realtor_metric_aliases`. The new shape has no `realtor_manager_id`; Phase 2 per-manager data lands in a separate table whenever a recurring source appears.
- **Realtor Corner → Real Estate Master Class.** "Realtor Corner" no longer exists in the 2026 source. Real Estate Master Class 1 and Master Class 2 are separate programs. Supervisor-locked 2026-05-25: UI renders one merged **"Weekly Realtor Meeting"** row that sums Master Class 1 + 2 per month, with a row-drill exposing the 1 vs 2 split. DB keeps both canonicals; merge happens at the UI layer in `mergeActivityForDisplay()`. Stakeholders Meeting attendance stays its own row.
- **NIL/Nil/NIl/Nll/Nl in the source = 0** (event happened, count was zero). Empty cell = null (no data). Non-numeric strings → coerced to 0 with `non_numeric_value` quality flag (raw value preserved for inspection).
- **Total mismatches surfaced not reconciled.** Computed week-sum wins for the fact-table `total` column; source's manual Total is preserved in `raw_row.Total`. Disagreement emits `total_mismatch` quality flag with both numbers; UI shows an amber chip on the metric row.
